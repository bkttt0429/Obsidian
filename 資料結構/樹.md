...
function morrisTraversal(root) {
    ...
    while (curr !== null) {
        if (curr.left === null) {
            steps.push(`節點 **${curr.val}** 沒有左子樹，加入結果，移至右側。`);
            ...
            if (pre.right === null) {
                // 建立線索
                steps.push(`建立線索：將節點 **${pre.val}** 的右指針指向 **${curr.val}**，進入左子樹。`);
                pre.right = curr;
                curr = curr.left;
            } else {
                // 斷開線索
                pre.right = null;
                steps.push(`斷開線索：節點 **${pre.val}** 重新指向 null，加入 **${curr.val}** 並移至右子樹。`);
                result.push(curr.val);
                curr = curr.right;
            }
        }
...
// 執行演算法
const { result, steps } = morrisTraversal(JSON.parse(JSON.stringify(tree)));

// 3. 在 Obsidian 中渲染介面
dv.header(2, "Morris Traversal 模擬過程");

dv.el("div", "這是一個 $O(1)$ 空間複雜度的中序遍歷演示。");
...
steps.forEach((step, i) => {
    dv.el("div", `> **Step ${i+1}:** ${step}`, { container });
});

dv.header(3, "最終遍歷結果");
dv.el("div", `\`${result.join(" → ")}\``, { 
    style: "font-size: 1.2em; font-weight: bold; color: var(--text-accent);" 
});
\```
```
---
