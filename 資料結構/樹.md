...
### Morris Traversal
> 一種空間複雜度為 O(1) 的中序遍歷演算法，不使用遞迴或堆疊。

#### 核心思想
Morris 遍歷的核心是利用樹中大量**空閒的右子指標**，在遍歷過程中建立**臨時的線索（Thread）**。這個線索會將當前節點 `curr` 的**中序前驅節點**（`curr` 左子樹的最右節點）的右子指標指向 `curr`。

當左子樹遍歷完成後，就可以沿著這個線索返回到 `curr`，從而避免使用堆疊。
...
2.  當 `current` 不為空時：
    -   **如果 `current` 沒有左子樹：**
        1.  訪問 `current`。
        2.  移動到右子樹 (`current = current->right`)。
    -   **如果 `current` 有左子樹：**
        1.  找到 `current` 的中序前驅 `pre`。
        2.  **如果 `pre` 的右子為空：**
            -   建立線索：`pre->right = current`。
            -   移動到左子樹：`current = current->left`。
        3.  **如果 `pre` 的右子已指向 `current`（線索已存在）：**
            -   斷開線索：`pre->right = NULL`。
            -   訪問 `current`。
            -   移動到右子樹：`current = current->right`。
          ...
          const steps = useMemo(() => {
            ...
            while (curr !== null) {
              s.push({
                ...
                threads: [...curThreads],
                res: [...res],
                msg: \`當前節點 (current) 為 \${curr.val}。檢查是否有左子樹。\`,
                highlight: 'check'
              });
              ...
              if (curr.left === null) {
                ...
                s.push({
                  ...
                  threads: [...curThreads],
                  res: [...res],
                  msg: \`\${curr.val} 沒有左子樹。訪問 \${curr.val}，移至右子樹 (current = current->right)。\`,
                  highlight: 'no-left'
                });
                ...
                s.push({
                    ...
                    threads: [...curThreads],
                    res: [...res],
                    msg: \`有左子樹。找到 \${curr.val} 的中序前驅 pre (\${pre.val})。\`,
                    highlight: 'find-pre'
                });
                ...
                if (getRight(pre, curThreads) === null) {
                  ...
                  s.push({
                    ...
                    threads: [...curThreads],
                    res: [...res],
                    msg: \`pre 的右子為空。建立線索 (pre->right = current)，移至左子樹 (current = current->left)。\`,
                    highlight: 'thread-creation'
                  });
                  curr = curr.left;
                } else {
                  // 3. 斷開線索
                  ...
                  s.push({
                    ...
                    threads: [...curThreads],
                    res: [...res],
                    msg: \`線索已存在。斷開線索 (pre->right = NULL)，訪問 \${curr.val}，移至右子樹 (current = current->right)。\`,
                    highlight: 'unthread'
                  });
                  curr = getRight(curr, curThreads);
                }
            ...
            s.push({ type: 'DONE', curr: null, pre: null, threads: [], res: [...res], msg: "Morris 遍歷完成！", highlight: 'done' });
            return s;
          }, []);
          ...
          return (
            <div className="flex flex-col h-full bg-neutral-50 p-6 font-sans text-neutral-800 rounded-xl border border-neutral-200 shadow-sm overflow-auto">
              ...
              <div className="flex flex-col lg:flex-row gap-4 flex-1 min-h-0">
                <div className="flex-1 bg-white rounded-xl border border-neutral-200 p-4 flex flex-col items-center justify-center relative overflow-hidden min-h-[400px]">
                ...
                <div className="w-full lg:w-[320px] flex flex-col space-y-4">
                  {/* 演算法程式碼 - 移至右區塊並根據圖片風格優化 */}
                  <div className="bg-neutral-900 rounded-xl p-4 shadow-xl border border-neutral-800 flex flex-col flex-1 min-h-[300px] overflow-hidden">
                    <h3 className="text-neutral-300 text-[11px] font-bold font-sans mb-4 tracking-wider flex items-center">
                      <span className="w-1.5 h-4 bg-blue-500 rounded-full mr-2"></span>
                      演算法程式碼
                    </h3>
                    <div className="space-y-0.5 text-[11px] font-mono leading-relaxed overflow-y-auto scrollbar-thin flex-1 pr-2 text-neutral-400">
                      {/* While Loop */}
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] ' + (steps[stepIndex]?.highlight === 'check' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        <span className="text-blue-400">while</span> (current != <span className="text-blue-400">null</span>) &#123;
                      </div>
                      
                      {/* If No Left */}
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-6 ' + (steps[stepIndex]?.highlight === 'no-left' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        <span className="text-blue-400">if</span> (current.left == <span className="text-blue-400">null</span>) &#123;
                      </div>
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-10 ' + (steps[stepIndex]?.highlight === 'no-left' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        visit(current);
                      </div>
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-10 ' + (steps[stepIndex]?.highlight === 'no-left' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                      ...
                      {/* Else Close/Start */}
                      <div className="pl-6 text-neutral-500">&#125; <span className="text-blue-400">else</span> &#123;</div>
                      
                      {/* Predecessor */}
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-10 ' + (steps[stepIndex]?.highlight === 'find-pre' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        pre = findInorderPredecessor(current);
                      </div>
                      
                      {/* If Thread Null */}
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-10 ' + (steps[stepIndex]?.highlight === 'thread-creation' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        <span className="text-blue-400">if</span> (pre.right == <span className="text-blue-400">null</span>) &#123;
                      </div>
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-14 ' + (steps[stepIndex]?.highlight === 'thread-creation' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        pre.right = current; <span className="text-emerald-500 ml-2 italic text-[10px]">// 建立線索</span>
                      </div>
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-14 ' + (steps[stepIndex]?.highlight === 'thread-creation' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        current = current.left;
                      </div>
                      
                      <div className="pl-10 text-neutral-500">&#125; <span className="text-blue-400">else</span> &#123;</div>
                      
                      {/* Unthread */}
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-14 ' + (steps[stepIndex]?.highlight === 'unthread' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        pre.right = <span className="text-blue-400">null</span>; <span className="text-emerald-500 ml-2 italic text-[10px]">// 移除線索</span>
                      </div>
                      <div className={'transition-all duration-300 py-0.5 px-3 -mx-3 border-l-[3px] pl-14 ' + (steps[stepIndex]?.highlight === 'unthread' ? 'bg-blue-500/10 border-blue-500 text-blue-300 font-bold' : 'border-transparent')}>
                        visit(current);
                      </div>
                      ...
                      {/* Closures */}
                      <div className="pl-10 text-neutral-500">&#125;</div>
                      <div className="pl-6 text-neutral-500">&#125;</div>
                      <div className="text-neutral-500">&#125;</div>
                    </div>
                  </div>

                  <div className="bg-neutral-900 rounded-xl p-4 shadow-lg min-h-[100px] max-h-[140px] overflow-y-auto scrollbar-thin border border-neutral-800">
                    <h3 className="text-neutral-500 text-[10px] font-mono mb-2 uppercase tracking-wider">執行步驟</h3>
                    <div className="space-y-1">
                      {logs.length > 0 ? (
                        logs.map((log, i) => (
                          <div key={i} className={'text-[11px] font-mono leading-tight ' + (i === 0 ? 'text-blue-400' : 'text-neutral-600')}>
                            {i === 0 ? '→ ' : '  '}{log}
                          </div>
                        ))
                      ) : (
                        <div className="text-neutral-700 italic text-[11px] font-mono">準備緒...</div>
                      )}
                    </div>
                  </div>

                  <div className="bg-white rounded-xl border border-neutral-200 p-4 shrink-0">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-wider text-neutral-400 font-sans">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {traversalResult.length > 0 ? (
                        traversalResult.map((v, i) => (
                          <span key={i} className="px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-[11px] font-mono font-bold border border-blue-100">
                            {v}
                          </span>
                        ))
                      ) : (
                        <span className="text-neutral-300 italic text-[10px]">暫無輸出</span>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 flex items-start space-x-3 p-3 bg-blue-50/50 border border-blue-100 rounded-lg">
                <Info size={16} className="text-blue-500 mt-0.5" />
                <div className="text-[11px] text-blue-800 leading-relaxed font-sans">
                  <span className="font-bold">設計概念：</span>
                  此模擬結合了樹狀結構變化與底層 C++ 邏輯。當前節點 (<span className="text-blue-600 font-bold">藍色</span>) 與前驅節點 (<span className="text-amber-500 font-bold">橙色</span>) 的互動即時反映在右側代碼高亮中。
                </div>
              </div>
            </div>
          );
...
dv.el("iframe", "", {
    attr: {
        srcdoc: htmlCode,
        style: "width: 100%; height: 850px; border: none; border-radius: 12px; background: white;"
    }
});
```
...
<!-- saved from url=(0141)blob:https://0kypcw6wli8hcfd6i8ss35ywo1azjj3qoq72nldkitudpiam78-h845251650.scf.usercontent.goog/97e4bf66-040e-4a6c-9ad3-7e0427a8bbd8#concepts -->
...
const IMG_GEN_REPLACE_MAP = {"official portrait of Claude Shannon information theory&reimagine=1": "https://lh3.googleusercontent.com/gg/AIJ2gl89i5l3JbicaWOFDTrqVLYAg295b_cdfgoX_JJoZWvgtgXQQbv-Q9Uzi7TBg6QjSm8N9sNMtEJJ0Hf6CCg29TeoxpV4EZ8CUE7c9SQ__-i-H51I_xiCsELaxAb91OB7YU3P2OhA1Xl5jJg2sF5i_I8yoH7y70DwjKFD9hl3kumEtqDOb93DverYlYKiIIBFelWawKS1cYvdGcRV4qZ3PzJ1EGOlL-WyGNUmft9PeOgGpSytIQe0SMzae4TNiKaDc6O4hnk-E0xHPq4h898oUNHjfUotJEUJOcTJEj90ht_T0C2YnI8FAjTazGYTI9r5zSzR3G1tG5O0Bh9abecjiwr0", "this image unchanged&context_image_idx=0": "https://lh3.googleusercontent.com/gg/AIJ2gl_yhiZ0CcNbzr6cQaNxCGRZHHzsFCJKEzXci8bzyV-Mam_bDQKKJeU8natU9g5TkWf7j9SN6bHEzO-lIAE9-i1dcq6Y_LyfaR380RdUXtfH9DjCJFmGFlGqYbKvf1XsZL7B3fhFoJUfkAG30HDxdx0bkXJKbj0LCxAMKgYlSbDXhK4Njg_D5JNn75jsaJjnwGChPFdTbVi5mFhU8yYqkfXoqd2aSY2-FRdiJ5hb1gXK_YP-BU2YzQbYL86bTgj9b5wMDSLyrN50CKq-HRyg_0WJTo6VeNHSIR4zbtTuHyPWuoiBubKYwzmS2j0nwOAy88QTiDm5okgmZ_JErXRA8a8", "abstract binary tree network pattern blue lines": "https://lh3.googleusercontent.com/gg/AIJ2gl_cv-9g4VU9nZRUOagtlp7sUuNtOaaxJdvGrKTCOQ7tCpPJebldNcOwlFSk32XLiEEuf8wOyT1ORXHhaMM1xzCp3G-H5Pmr8Y8LDOoTxCHZDFJ_tGxe2EQ-ppPMmMBD_Z_IBuz93ivQiYS_iIFOxuoWuO2tQ3JSqj7tCVUTS-dAGN9yFH1q-HR5R3cbSjLGzE02rF2umu8tlS29yNDMMnYcdjf5WFRMNWii8YaS1WSF7px0BD-zG-YXZHM8OkQCuvYBwHw-Ya8ivyP1Dd7uk10y3Z2CFyZ7McmoJBwTMv5kwBKOEVfIodEhKlKcVbApbux0dkRJx4e135igk8ZbM5BM", "a clean minimalist visualization of data compression from many boxes into a small efficient box&aspect=4:3": "https://lh3.googleusercontent.com/gg/AIJ2gl_689qRufA68Mq-ZPwd3OnAfPSXrI6vcM8-QFojl-uorSh3eKpIBOzrf9ofYged7WJz30_u3jpURjHdZBqSp4ST7ealcuRJpvLhlwtOSLk8msAE-9TpV6HKywYNJDd63A_WufcOXmzPSwlAucw58JQsP0RykPYpUL5swPu1biTMci-syByHMOn3m7gVeRIa2RIXI6rT3LPq-71_ISEBT2zbShsx3ubb1kJkg2Oh3YZNlMc1FucAWYYUGtYIXZtbpcjnJM5UwLK9KOHpSs9Jngf8OoZv5H6SAcQqB3k2OGsGpnsuuOAonCIkRWpix1drsu6Qj104sPnZ-SsnmNVXMuA", "abstract mathematical concept growth of tree nodes connecting glowing points&aspect=16:9": "https://lh3.googleusercontent.com/gg/AIJ2gl_7m4Zt3ZB8dBcAYQ7-OIP6eecD5a0k-oFaEuBsxBriLpDUVphvUB1tAfi6QE-PaFYnPZNjlz-zRT34RimdKqmp4E5gkl-I-ylyvIKiPRC-I_zJ0jcUgKdjvu560hLcfwKJnnhOj3rK15EUbv_7_skoguprgMqur9JoTVdKAZeqJKJNldexU5T6lvUbpJVdypJbm00tCzQLBuEwv1KYpEzKmvwhNkzk5SzFTm0iQU99eEWgtexZLsc7JCprQLXjPO7G6aSms_mX5vI_j7HERfxxrY544ELQmsXiOpnGF7mQMJ_ckNd95A26hSHzwGfE6WBYFvTguS66gNVZ1L8o86Ip", "minimalist diagram showing entropy and efficiency curve data compression&aspect=16:9": "https://lh3.googleusercontent.com/gg/AIJ2gl_J1T5YH4-jXt1QpAdRx-6S-Ir7aLwwPyaZeVTBR0ALdUpC7r9xVFe87tPl0VcrCU1oZGXOPP9OXg6HPigSIhYMbweUwRx4l72xXWT01StoZOduch2O5X3YedawKJ5TG-j84SUAXF4joRi2GyqnzKq3lWVkBxeKhyntc95O9YljSOTnUnrkpk-c93yMOLixWdRJ8Mvy4-9wmI9Jb2P83vuLTD9UwEUYNDavVOQrTbeCkpWVemKgsmzDsaq55-0Ft15xC5tD9sU2VMq1NuZ7vgFEqF8Fat3DUUkgQgE0uoY7p07jue8DYTi7VL-9XnPBngFEI10GgJMSTOnHIFCJxYjo"};
...
wb);O(IMG_SEARCH_ATTRIBUTION_MAP,Pd);O(IMG_GEN_REPLACE_MAP,fa.A);X();jf();h.A=0})})();document.readyState==="loading"?document.addEventListener("DOMContentLoaded",function(){Zd()}):Zd()})();}).call(this);
    ...
    <style>
        ...
        .tree-line { stroke: #cbd5e1; stroke-width: 2; fill: none; }
        .node-circle { fill: #fff; stroke: #3b82f6; stroke-width: 2; }
        .node-leaf { fill: #ecfdf5; stroke: #10b981; }
        .node-text { font-size: 12px; font-weight: 600; fill: #1e293b; pointer-events: none; }
        .edge-label { font-size: 14px; font-weight: bold; fill: #ef4444; }
...
<style>*, ::before, ::after{--tw-border-spacing-x:0;--tw-border-spacing-y:0;--tw-translate-x:0;--tw-translate-y:0;--tw-rotate:0;--tw-skew-x:0;--tw-skew-y:0;--tw-scale-x:1;--tw-scale-y:1;--tw-pan-x: ;--tw-pan-y: ;--tw-pinch-zoom: ;--tw-scroll-snap-strictness:proximity;--tw-gradient-from-position: ;--tw-gradient-via-position: ;--tw-gradient-to-position: ;--tw-ordinal: ;--tw-slashed-zero: ;--tw-numeric-figure: ;--tw-numeric-spacing: ;--tw-numeric-fraction: ;--tw-ring-inset: ;--tw-ring-offset-width:0px;--tw-ring-offset-color:#fff;--tw-ring-color:rgb(59 130 246 / 0.5);--tw-ring-offset-shadow:0 0 #0000;--tw-ring-shadow:0 0 #0000;--tw-shadow:0 0 #0000;--tw-shadow-colored:0 0 #0000;--tw-blur: ;--tw-brightness: ;--tw-contrast: ;--tw-grayscale: ;--tw-hue-rotate: ;--tw-invert: ;--tw-saturate: ;--tw-sepia: ;--tw-drop-shadow: ;--tw-backdrop-blur: ;--tw-backdrop-brightness: ;--tw-backdrop-contrast: ;--tw-backdrop-grayscale: ;--tw-backdrop-hue-rotate: ;--tw-backdrop-invert: ;--tw-backdrop-opacity: ;--tw-backdrop-saturate: ;--tw-backdrop-sepia: ;--tw-contain-size: ;--tw-contain-layout: ;--tw-contain-paint: ;--tw-contain-style: }::backdrop{--tw-border-spacing-x:0;--tw-border-spacing-y:0;--tw-translate-x:0;--tw-translate-y:0;--tw-rotate:0;--tw-skew-x:0;--tw-skew-y:0;--tw-scale-x:1;--tw-scale-y:1;--tw-pan-x: ;--tw-pan-y: ;--tw-pinch-zoom: ;--tw-scroll-snap-strictness:proximity;--tw-gradient-from-position: ;--tw-gradient-via-position: ;--tw-gradient-to-position: ;--tw-ordinal: ;--tw-slashed-zero: ;--tw-numeric-figure: ;--tw-numeric-spacing: ;--tw-numeric-fraction: ;--tw-ring-inset: ;--tw-ring-offset-width:0px;--tw-ring-offset-color:#fff;--tw-ring-color:rgb(59 130 246 / 0.5);--tw-ring-offset-shadow:0 0 #0000;--tw-ring-shadow:0 0 #0000;--tw-shadow:0 0 #0000;--tw-shadow-colored:0 0 #0000;--tw-blur: ;--tw-brightness: ;--tw-contrast: ;--tw-grayscale: ;--tw-hue-rotate: ;--tw-invert: ;--tw-saturate: ;--tw-sepia: ;--tw-drop-shadow: ;--tw-backdrop-blur: ;--tw-backdrop-brightness: ;--tw-backdrop-contrast: ;--tw-backdrop-grayscale: ;--tw-backdrop-hue-rotate: ;--tw-backdrop-invert: ;--tw-backdrop-opacity: ;--tw-backdrop-saturate: ;--tw-backdrop-sepia: ;--tw-contain-size: ;--tw-contain-layout: ;--tw-contain-paint: ;--tw-contain-style: }/* ! tailwindcss v3.4.17 | MIT License | https://tailwindcss.com */*,::after,::before{box-sizing:border-box;border-width:0;border-style:solid;border-color:#e5e7eb}::after,::before{--tw-content:''}:host,html{line-height:1.5;-webkit-text-size-adjust:100%;-moz-tab-size:4;tab-size:4;font-family:ui-sans-serif, system-ui, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji";font-feature-settings:normal;font-variation-settings:normal;-webkit-tap-highlight-color:transparent}body{margin:0;line-height:inherit}hr{height:0;color:inherit;border-top-width:1px}abbr:where([title]){-webkit-text-decoration:underline dotted;text-decoration:underline dotted}h1,h2,h3,h4,h5,h6{font-size:inherit;font-weight:inherit}a{color:inherit;text-decoration:inherit}b,strong{font-weight:bolder}code,kbd,pre,samp{font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace;font-feature-settings:normal;font-variation-settings:normal;font-size:1em}small{font-size:80%}sub,sup{font-size:75%;line-height:0;position:relative;vertical-align:baseline}sub{bottom:-.25em}sup{top:-.5em}table{text-indent:0;border-color:inherit;border-collapse:collapse}button,input,optgroup,select,textarea{font-family:inherit;font-feature-settings:inherit;font-variation-settings:inherit;font-size:100%;font-weight:inherit;line-height:inherit;letter-spacing:inherit;color:inherit;margin:0;padding:0}button,select{text-transform:none}button,input:where([type=button]),input:where([type=reset]),input:where([type=submit]){-webkit-appearance:button;background-color:transparent;background-image:none}:-moz-focusring{outline:auto}:-moz-ui-invalid{box-shadow:none}progress{vertical-align:baseline}::-webkit-inner-spin-button,::-webkit-outer-spin-button{height:auto}[type=search]{-webkit-appearance:textfield;outline-offset:-2px}::-webkit-search-decoration{-webkit-appearance:none}::-webkit-file-upload-button{-webkit-appearance:button;font:inherit}summary{display:list-item}blockquote,dd,dl,figure,h1,h2,h3,h4,h5,h6,hr,p,pre{margin:0}fieldset{margin:0;padding:0}legend{padding:0}menu,ol,ul{list-style:none;margin:0;padding:0}dialog{padding:0}textarea{resize:vertical}input::placeholder,textarea::placeholder{opacity:1;color:#9ca3af}[role=button],button{cursor:pointer}:disabled{cursor:default}audio,canvas,embed,iframe,img,object,svg,video{display:block;vertical-align:middle}img,video{max-width:100%;height:auto}[hidden]:where(:not([hidden=until-found])){display:none}.pointer-events-none{pointer-events:none}.absolute{position:absolute}.relative{position:relative}.inset-0{inset:0px}.bottom-0{bottom:0px}.right-0{right:0px}.top-0{top:0px}.z-10{z-index:10}.order-1{order:1}.order-2{order:2}.mx-auto{margin-left:auto;margin-right:auto}.mb-1{margin-bottom:0.25rem}.mb-12{margin-bottom:3rem}.mb-2{margin-bottom:0.5rem}.mb-4{margin-bottom:1rem}.mb-6{margin-bottom:1.5rem}.mb-8{margin-bottom:2rem}.mr-1{margin-right:0.25rem}.mr-2{margin-right:0.5rem}.mr-3{margin-right:0.75rem}.mt-2{margin-top:0.5rem}.mt-4{margin-top:1rem}.mt-6{margin-top:1.5rem}.flex{display:flex}.inline-flex{display:inline-flex}.grid{display:grid}.hidden{display:none}.h-12{height:3rem}.h-8{height:2rem}.h-\\[600px\\]{height:600px}.h-full{height:100%}.min-h-\\[500px\\]{min-height:500px}.w-12{width:3rem}.w-8{width:2rem}.w-\\[600px\\]{width:600px}.w-full{width:100%}.max-w-7xl{max-width:80rem}.max-w-md{max-width:28rem}.flex-1{flex:1 1 0%}.flex-grow{flex-grow:1}.-translate-y-1\\/2{--tw-translate-y:-50%;transform:translate(var(--tw-translate-x), var(--tw-translate-y)) rotate(var(--tw-rotate)) skewX(var(--tw-skew-x)) skewY(var(--tw-skew-y)) scaleX(var(--tw-scale-x)) scaleY(var(--tw-scale-y))}.translate-x-1\\/2{--tw-translate-x:50%;transform:translate(var(--tw-translate-x), var(--tw-translate-y)) rotate(var(--tw-rotate)) skewX(var(--tw-skew-x)) skewY(var(--tw-skew-y)) scaleX(var(--tw-scale-x)) scaleY(var(--tw-scale-y))}.scroll-mt-12{scroll-margin-top:3rem}.grid-cols-1{grid-template-columns:repeat(1, minmax(0, 1fr))}.flex-col{flex-direction:column}.flex-wrap{flex-wrap:wrap}.items-center{align-items:center}.justify-center{justify-content:center}.justify-between{justify-content:space-between}.gap-12{gap:3rem}.gap-4{gap:1rem}.gap-6{gap:1.5rem}.gap-8{gap:2rem}.gap-2{gap:0.5rem}.space-y-24 > :not([hidden]) ~ :not([hidden]){--tw-space-y-reverse:0;margin-top:calc(6rem * calc(1 - var(--tw-space-y-reverse)));margin-bottom:calc(6rem * var(--tw-space-y-reverse))}.space-y-3 > :not([hidden]) ~ :not([hidden]){--tw-space-y-reverse:0;margin-top:calc(0.75rem * calc(1 - var(--tw-space-y-reverse)));margin-bottom:calc(0.75rem * var(--tw-space-y-reverse))}.space-y-4 > :not([hidden]) ~ :not([hidden]){--tw-space-y-reverse:0;margin-top:calc(1rem * calc(1 - var(--tw-space-y-reverse)));margin-bottom:calc(1rem * var(--tw-space-y-reverse))}.space-y-6 > :not([hidden]) ~ :not([hidden]){--tw-space-y-reverse:0;margin-top:calc(1.5rem * calc(1 - var(--tw-space-y-reverse)));margin-bottom:calc(1.5rem * var(--tw-space-y-reverse))}.divide-y > :not([hidden]) ~ :not([hidden]){--tw-divide-y-reverse:0;border-top-width:calc(1px * calc(1 - var(--tw-divide-y-reverse)));border-bottom-width:calc(1px * var(--tw-divide-y-reverse))}.divide-slate-100 > :not([hidden]) ~ :not([hidden]){--tw-divide-opacity:1;border-color:rgb(241 245 249 / var(--tw-divide-opacity, 1))}.overflow-auto{overflow:auto}.overflow-hidden{overflow:hidden}.overflow-x-auto{overflow-x:auto}.rounded{border-radius:0.25rem}.rounded-2xl{border-radius:1rem}.rounded-3xl{border-radius:1.5rem}.rounded-full{border-radius:9999px}.rounded-lg{border-radius:0.5rem}.rounded-xl{border-radius:0.75rem}.rounded-r-lg{border-top-right-radius:0.5rem;border-bottom-right-radius:0.5rem}.border{border-width:1px}.border-2{border-width:2px}.border-b{border-bottom-width:1px}.border-l-4{border-left-width:4px}.border-dashed{border-style:dashed}.border-amber-400{--tw-border-opacity:1;border-color:rgb(251 191 36 / var(--tw-border-opacity, 1))}.border-blue-100{--tw-border-opacity:1;border-color:rgb(219 234 254 / var(--tw-border-opacity, 1))}.border-blue-200{--tw-border-opacity:1;border-color:rgb(191 219 254 / var(--tw-border-opacity, 1))}.border-slate-100{--tw-border-opacity:1;border-color:rgb(241 245 249 / var(--tw-border-opacity, 1))}.border-slate-200{--tw-border-opacity:1;border-color:rgb(226 232 240 / var(--tw-border-opacity, 1))}.border-slate-300{--tw-border-opacity:1;border-color:rgb(203 213 225 / var(--tw-border-opacity, 1))}.bg-amber-50{--tw-bg-opacity:1;background-color:rgb(255 251 235 / var(--tw-bg-opacity, 1))}.bg-blue-100{--tw-bg-opacity:1;background-color:rgb(219 234 254 / var(--tw-bg-opacity, 1))}.bg-blue-50{--tw-bg-opacity:1;background-color:rgb(239 246 255 / var(--tw-bg-opacity, 1))}.bg-blue-500{--tw-bg-opacity:1;background-color:rgb(59 130 246 / var(--tw-bg-opacity, 1))}.bg-blue-600{--tw-bg-opacity:1;background-color:rgb(37 99 235 / var(--tw-bg-opacity, 1))}.bg-green-600{--tw-bg-opacity:1;background-color:rgb(22 163 74 / var(--tw-bg-opacity, 1))}.bg-slate-100{--tw-bg-opacity:1;background-color:rgb(241 245 249 / var(--tw-bg-opacity, 1))}.bg-slate-50{--tw-bg-opacity:1;background-color:rgb(248 250 252 / var(--tw-bg-opacity, 1))}.bg-slate-800{--tw-bg-opacity:1;background-color:rgb(30 41 59 / var(--tw-bg-opacity, 1))}.bg-slate-900{--tw-bg-opacity:1;background-color:rgb(15 23 42 / var(--tw-bg-opacity, 1))}.bg-white{--tw-bg-opacity:1;background-color:rgb(255 255 255 / var(--tw-bg-opacity, 1))}.bg-white\\/80{background-color:rgb(255 255 255 / 0.8)}.p-3{padding:0.75rem}.p-4{padding:1rem}.p-6{padding:1.5rem}.p-8{padding:2rem}.px-2{padding-left:0.5rem;padding-right:0.5rem}.px-3{padding-left:0.75rem;padding-right:0.75rem}.px-4{padding-left:1rem;padding-right:1rem}.px-6{padding-left:1.5rem;padding-right:1.5rem}.py-1{padding-top:0.25rem;padding-bottom:0.25rem}.py-12{padding-top:3rem;padding-bottom:3rem}.py-2{padding-top:0.5rem;padding-bottom:0.5rem}.py-3{padding-top:0.75rem;padding-bottom:0.75rem}.pb-24{padding-bottom:6rem}.text-left{text-align:left}.text-center{text-align:center}.font-mono{font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace}.text-2xl{font-size:1.5rem;line-height:2rem}.text-3xl{font-size:1.875rem;line-height:2.25rem}.text-4xl{font-size:2.25rem;line-height:2.5rem}.text-lg{font-size:1.125rem;line-height:1.75rem}.text-sm{font-size:0.875rem;line-height:1.25rem}.text-xl{font-size:1.25rem;line-height:1.75rem}.text-xs{font-size:0.75rem;line-height:1rem}.font-black{font-weight:900}.font-bold{font-weight:700}.font-extrabold{font-weight:800}.font-medium{font-weight:500}.font-semibold{font-weight:600}.font-normal{font-weight:400}.uppercase{text-transform:uppercase}.italic{font-style:italic}.leading-relaxed{line-height:1.625}.tracking-tight{letter-spacing:-0.025em}.text-amber-700{--tw-text-opacity:1;color:rgb(180 83 9 / var(--tw-text-opacity, 1))}.text-amber-800{--tw-text-opacity:1;color:rgb(146 64 14 / var(--tw-text-opacity, 1))}.text-blue-500{--tw-text-opacity:1;color:rgb(59 130 246 / var(--tw-text-opacity, 1))}.text-blue-600{--tw-text-opacity:1;color:rgb(37 99 235 / var(--tw-text-opacity, 1))}.text-blue-700{--tw-text-opacity:1;color:rgb(29 78 216 / var(--tw-text-opacity, 1))}.text-blue-800{--tw-text-opacity:1;color:rgb(30 64 175 / var(--tw-text-opacity, 1))}.text-blue-900{--tw-text-opacity:1;color:rgb(30 58 138 / var(--tw-text-opacity, 1))}.text-green-500{--tw-text-opacity:1;color:rgb(34 197 94 / var(--tw-text-opacity, 1))}.text-purple-500{--tw-text-opacity:1;color:rgb(168 85 247 / var(--tw-text-opacity, 1))}.text-red-500{--tw-text-opacity:1;color:rgb(239 68 68 / var(--tw-text-opacity, 1))}.text-slate-400{--tw-text-opacity:1;color:rgb(148 163 184 / var(--tw-text-opacity, 1))}.text-slate-500{--tw-text-opacity:1;color:rgb(100 116 139 / var(--tw-text-opacity, 1))}.text-slate-600{--tw-text-opacity:1;color:rgb(71 85 105 / var(--tw-text-opacity, 1))}.text-slate-700{--tw-text-opacity:1;color:rgb(51 65 85 / var(--tw-text-opacity, 1))}.text-slate-800{--tw-text-opacity:1;color:rgb(30 41 59 / var(--tw-text-opacity, 1))}.text-slate-900{--tw-text-opacity:1;color:rgb(15 23 42 / var(--tw-text-opacity, 1))}.text-white{--tw-text-opacity:1;color:rgb(255 255 255 / var(--tw-text-opacity, 1))}.opacity-10{opacity:0.1}.opacity-20{opacity:0.2}.opacity-75{opacity:0.75}.shadow-2xl{--tw-shadow:0 25px 50px -12px rgb(0 0 0 / 0.25);--tw-shadow-colored:0 25px 50px -12px var(--tw-shadow-color);box-shadow:var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow)}.shadow-lg{--tw-shadow:0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1);--tw-shadow-colored:0 10px 15px -3px var(--tw-shadow-color), 0 4px 6px -4px var(--tw-shadow-color);box-shadow:var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow)}.shadow-sm{--tw-shadow:0 1px 2px 0 rgb(0 0 0 / 0.05);--tw-shadow-colored:0 1px 2px 0 var(--tw-shadow-color);box-shadow:var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow)}.outline-none{outline:2px solid transparent;outline-offset:2px}.transition{transition-property:color, background-color, border-color, fill, stroke, opacity, box-shadow, transform, filter, -webkit-text-decoration-color, -webkit-backdrop-filter;transition-property:color, background-color, border-color, text-decoration-color, fill, stroke, opacity, box-shadow, transform, filter, backdrop-filter;transition-property:color, background-color, border-color, text-decoration-color, fill, stroke, opacity, box-shadow, transform, filter, backdrop-filter, -webkit-text-decoration-color, -webkit-backdrop-filter;transition-timing-function:cubic-bezier(0.4, 0, 0.2, 1);transition-duration:150ms}.hover\\:bg-blue-700:hover{--tw-bg-opacity:1;background-color:rgb(29 78 216 / var(--tw-bg-opacity, 1))}.hover\\:bg-slate-50:hover{--tw-bg-opacity:1;background-color:rgb(248 250 252 / var(--tw-bg-opacity, 1))}.hover\\:text-red-500:hover{--tw-text-opacity:1;color:rgb(239 68 68 / var(--tw-text-opacity, 1))}.hover\\:shadow-md:hover{--tw-shadow:0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1);--tw-shadow-colored:0 4px 6px -1px var(--tw-shadow-color), 0 2px 4px -2px var(--tw-shadow-color);box-shadow:var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow)}.focus\\:ring-2:focus{--tw-ring-offset-shadow:var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color);--tw-ring-shadow:var(--tw-ring-inset) 0 0 0 calc(2px + var(--tw-ring-offset-width)) var(--tw-ring-color);box-shadow:var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000)}.focus\\:ring-blue-500:focus{--tw-ring-opacity:1;--tw-ring-color:rgb(59 130 246 / var(--tw-ring-opacity, 1))}@media (min-width: 640px){.sm\\:grid-cols-2{grid-template-columns:repeat(2, minmax(0, 1fr))}.sm\\:grid-cols-3{grid-template-columns:repeat(3, minmax(0, 1fr))}.sm\\:px-6{padding-left:1.5rem;padding-right:1.5rem}.sm\\:text-5xl{font-size:3rem;line-height:1}}@media (min-width: 768px){.md\\:order-1{order:1}.md\\:order-2{order:2}.md\\:grid-cols-2{grid-template-columns:repeat(2, minmax(0, 1fr))}.md\\:grid-cols-3{grid-template-columns:repeat(3, minmax(0, 1fr))}.md\\:flex-row{flex-direction:row}.md\\:p-12{padding:3rem}}@media (min-width: 1024px){.lg\\:col-span-4{grid-column:span 4 / span 4}.lg\\:col-span-8{grid-column:span 8 / span 8}.lg\\:grid-cols-12{grid-template-columns:repeat(12, minmax(0, 1fr))}.lg\\:grid-cols-4{grid-template-columns:repeat(4, minmax(0, 1fr))}.lg\\:px-8{padding-left:2rem;padding-right:2rem}}</style><link type="text/css" href="https://fonts.googleapis.com/css2?family=Google+Symbols:opsz,wght,FILL,GRAD,ROND@24,400,0,0,50&amp;icon_names=link" rel="stylesheet"><style id="dyn-img-link-styles-di-script">
      .dyn-img-link-wrapper-di-script {
        anchor-scope: all;
        display: contents;
        position: relative;
...
<body class="text-slate-800">
    ...
    <header class="relative bg-white border-b border-slate-200 overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center gap-12 relative z-10">
            <div class="flex-1">
                <div class="inline-flex items-center px-3 py-1 rounded-full bg-blue-50 text-blue-600 text-sm font-medium mb-4">
                    <span class="material-symbols-outlined text-sm mr-1">compress</span> 数据压缩核心算法
                </div>
                <h1 class="text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl mb-6">
                    哈夫曼树 (Huffman Tree)
                </h1>
                <p class="text-lg text-slate-600 mb-8 leading-relaxed">
                    哈夫曼树，又称最优二叉树，是给定 $n$ 个权值作为 $n$ 个叶子节点，构造一棵二叉树，若该树的带权路径长度 (WPL) 达到最小，称这样的二叉树为最优二叉树。它是现代无损数据压缩（如 ZIP、JPEG）的基石。
                </p>
                <div class="flex flex-wrap gap-4">
                    <a href="blob:https://0kypcw6wli8hcfd6i8ss35ywo1azjj3qoq72nldkitudpiam78-h845251650.scf.usercontent.goog/97e4bf66-040e-4a6c-9ad3-7e0427a8bbd8#interactive-tool" class="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition shadow-lg">立即体验交互工具</a>
                    <a href="blob:https://0kypcw6wli8hcfd6i8ss35ywo1azjj3qoq72nldkitudpiam78-h845251650.scf.usercontent.goog/97e4bf66-040e-4a6c-9ad3-7e0427a8bbd8#concepts" class="px-6 py-3 bg-white border border-slate-300 text-slate-700 font-semibold rounded-lg hover:bg-slate-50 transition">了解理论知识</a>
                </div>
            </div>
            <div class="flex-1 w-full max-w-md">
                <div class="bg-white p-4 rounded-2xl shadow-2xl border border-slate-100">
                    <img src="blob:https://0kypcw6wli8hcfd6i8ss35ywo1azjj3qoq72nldkitudpiam78-h845251650.scf.usercontent.goog/3c8ea4f7-35b9-493e-82bf-9e7933edb648" go-data-src="/gen?prompt=this+image+unchanged&amp;context_image_idx=0" alt="哈夫曼树原始案例" class="rounded-lg w-full" style="">
                    <p class="text-xs text-center text-slate-400 mt-2 italic">基于您上传的案例：100题选项分布与压缩对比</p>
                </div>
            </div>
        </div>
        <!-- Abstract Background -->
        <div class="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 opacity-10 pointer-events-none">
            <img style="" src="blob:https://0kypcw6wli8hcfd6i8ss35ywo1azjj3qoq72nldkitudpiam78-h845251650.scf.usercontent.goog/65c4e877-8371-4e8c-9963-e4d851f21263" go-data-src="/gen?prompt=abstract+binary+tree+network+pattern+blue+lines&amp;aspect=1:1" class="w-[600px] h-[600px]">
        </div>
    </header>

    <main class="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8 space-y-24">

        <!-- Section 1: Introduction -->
        <section id="concepts" class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
                ...
                <div class="space-y-4 text-slate-600 leading-relaxed">
                    <p>
                        在传统的<strong>固定长度编码</strong>中，每个字符（如 A, B, C, D）都占用相同数量的二进制位（比如都是 2 位）。这虽然简单，但如果某些字符出现的频率非常高，就会造成空间浪费。
                    </p>
                    <p>
                        <strong>哈夫曼编码</strong> 采用变长编码策略：给频率高的字符分配短的编码，频率低的分配长的编码。
                    </p>
                    <div class="bg-amber-50 border-l-4 border-amber-400 p-4 rounded-r-lg mt-4">
                        <p class="text-amber-800 font-medium">核心优势：前缀特性</p>
                        <p class="text-sm text-amber-700">哈夫曼编码是一组“前缀编码”，即任何字符的编码都不是另一个字符编码的前缀。这保证了解码时不会产生歧义。</p>
                    </div>
                </div>
            </div>
            <div class="relative">
                <img style="" src="blob:https://0kypcw6wli8hcfd6i8ss35ywo1azjj3qoq72nldkitudpiam78-h845251650.scf.usercontent.goog/0e4550db-3bdc-4100-8a64-21dfc8e32142" go-data-src="/gen?prompt=a+clean+minimalist+visualization+of+data+compression+from+many+boxes+into+a+small+efficient+box&amp;aspect=4:3" class="rounded-2xl shadow-lg border border-slate-200">
            </div>
        </section>

        <!-- Section 2: Algorithm Steps -->
        <section class="bg-slate-900 rounded-3xl p-8 md:p-12 text-white overflow-hidden relative">
            <div class="relative z-10">
                <h2 class="text-3xl font-bold mb-8">哈夫曼树构建过程</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="space-y-4">
                        <div class="w-12 h-12 rounded-full bg-blue-500 flex items-center justify-center font-bold text-xl">1</div>
                        <h3 class="text-xl font-semibold">初始化</h3>
                        <p class="text-slate-400 text-sm">将每个符号看作一个独立的森林节点，权值为其出现的频率。</p>
                    </div>
                    <div class="space-y-4">
                        <div class="w-12 h-12 rounded-full bg-blue-500 flex items-center justify-center font-bold text-xl">2</div>
                        <h3 class="text-xl font-semibold">合并最小节点</h3>
                        <p class="text-slate-400 text-sm">从森林中选取两个权值最小的节点作为左右孩子，创建一个新节点作为父节点，其权值为左右孩子权值之和。</p>
                    </div>
                    <div class="space-y-4">
                        <div class="w-12 h-12 rounded-full bg-blue-500 flex items-center justify-center font-bold text-xl">3</div>
                        <h3 class="text-xl font-semibold">循环往复</h3>
                        <p class="text-slate-400 text-sm">将新节点加入森林，重复步骤2，直到森林中只剩下一个根节点为止。</p>
                    </div>
                </div>
            </div>
            <div class="absolute bottom-0 right-0 opacity-20">
                <img style="" src="blob:https://0kypcw6wli8hcfd6i8ss35ywo1azjj3qoq72nldkitudpiam78-h845251650.scf.usercontent.goog/20ab958e-89a4-49f6-b8f6-e8d841658ff0" go-data-src="/gen?prompt=abstract+mathematical+concept+growth+of+tree+nodes+connecting+glowing+points&amp;aspect=16:9" class="w-full">
            </div>
        </section>

        <!-- Section 3: Interactive Tool -->
        <section id="interactive-tool" class="scroll-mt-12">
            <div class="text-center mb-12">
                <h2 class="text-3xl font-bold text-slate-900">哈夫曼树实时构建器</h2>
                <p class="text-slate-500 mt-2">修改左侧权值，右侧树结构和编码表将实时更新</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        <h3 class="font-bold text-lg mb-4 flex items-center">
                            <span class="material-symbols-outlined mr-2 text-blue-500">list</span> 数据输入
                        </h3>
                        <div id="input-list" class="space-y-3 mb-6"><div class="flex gap-2 items-center">
                    <input type="text" value="C" maxlength="1" oninput="updateData(0, &#39;char&#39;, this.value)" class="w-12 px-2 py-2 border rounded-lg text-center font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                    <input type="number" value="80" oninput="updateData(0, &#39;weight&#39;, this.value)" class="flex-grow px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    <button onclick="removeRow(0)" class="text-slate-400 hover:text-red-500 transition">
                        <span class="material-symbols-outlined">delete</span>
                    </button>
                </div><div class="flex gap-2 items-center">
                    <input type="text" value="A" maxlength="1" oninput="updateData(1, &#39;char&#39;, this.value)" class="w-12 px-2 py-2 border rounded-lg text-center font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                    <input type="number" value="10" oninput="updateData(1, &#39;weight&#39;, this.value)" class="flex-grow px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    <button onclick="removeRow(1)" class="text-slate-400 hover:text-red-500 transition">
                        <span class="material-symbols-outlined">delete</span>
                    </button>
                </div><div class="flex gap-2 items-center">
                    <input type="text" value="B" maxlength="1" oninput="updateData(2, &#39;char&#39;, this.value)" class="w-12 px-2 py-2 border rounded-lg text-center font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                    <input type="number" value="8" oninput="updateData(2, &#39;weight&#39;, this.value)" class="flex-grow px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    <button onclick="removeRow(2)" class="text-slate-400 hover:text-red-500 transition">
                        <span class="material-symbols-outlined">delete</span>
                    </button>
                </div><div class="flex gap-2 items-center">
                    <input type="text" value="D" maxlength="1" oninput="updateData(3, &#39;char&#39;, this.value)" class="w-12 px-2 py-2 border rounded-lg text-center font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                    <input type="number" value="2" oninput="updateData(3, &#39;weight&#39;, this.value)" class="flex-grow px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    <button onclick="removeRow(3)" class="text-slate-400 hover:text-red-500 transition">
                        <span class="material-symbols-outlined">delete</span>
                    </button>
                </div></div>
                        <button onclick="addRow()" class="w-full py-2 border-2 border-dashed border-slate-300 rounded-lg text-slate-500 hover:bg-slate-50 transition flex items-center justify-center">
                            <span class="material-symbols-outlined mr-1">add</span> 添加字符
                        </button>
                    </div>

                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        <h3 class="font-bold text-lg mb-4 flex items-center">
                            <span class="material-symbols-outlined mr-2 text-green-500">table_chart</span> 编码对比表
                        </h3>
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm text-left">
                                <thead class="bg-slate-50 text-slate-500 uppercase">
                                    <tr>
                                        <th class="px-3 py-2">字符</th>
                                        <th class="px-3 py-2">固定码</th>
                                        <th class="px-3 py-2">哈夫曼码</th>
                                    </tr>
                                </thead>
                                <tbody id="result-table-body" class="divide-y divide-slate-100"><tr>
                    <td class="px-3 py-2 font-bold">C <span class="text-xs font-normal text-slate-400">(80)</span></td>
                    <td class="px-3 py-2 font-mono text-slate-400">11</td>
                    <td class="px-3 py-2 font-mono text-blue-600 font-bold">1</td>
                </tr><tr>
                    <td class="px-3 py-2 font-bold">A <span class="text-xs font-normal text-slate-400">(10)</span></td>
                    <td class="px-3 py-2 font-mono text-slate-400">01</td>
                    <td class="px-3 py-2 font-mono text-blue-600 font-bold">00</td>
                </tr><tr>
                    <td class="px-3 py-2 font-bold">B <span class="text-xs font-normal text-slate-400">(8)</span></td>
                    <td class="px-3 py-2 font-mono text-slate-400">10</td>
                    <td class="px-3 py-2 font-mono text-blue-600 font-bold">011</td>
                </tr><tr>
                    <td class="px-3 py-2 font-bold">D <span class="text-xs font-normal text-slate-400">(2)</span></td>
                    <td class="px-3 py-2 font-mono text-slate-400">00</td>
                    <td class="px-3 py-2 font-mono text-blue-600 font-bold">010</td>
                </tr></tbody>
                            </table>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-8 flex flex-col gap-6">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex-grow relative overflow-hidden min-h-[500px]">
                        <h3 class="font-bold text-lg mb-4 flex items-center">
                            <span class="material-symbols-outlined mr-2 text-purple-500">account_tree</span> 动态树视图
                        </h3>
                        <div id="tree-container" class="w-full h-full flex items-center justify-center overflow-auto">
                            <svg id="huffman-svg" width="600" height="400" viewBox="0 0 600 400" preserveAspectRatio="xMidYMid meet"><line x1="300" y1="40" x2="150" y2="110" class="tree-line"></line><text x="215" y="75" class="edge-label">0</text><line x1="150" y1="110" x2="75" y2="180" class="tree-line"></line><text x="102.5" y="145" class="edge-label">0</text><line x1="150" y1="110" x2="225" y2="180" class="tree-line"></line><text x="177.5" y="145" class="edge-label">1</text><line x1="225" y1="180" x2="187.5" y2="250" class="tree-line"></line><text x="196.25" y="215" class="edge-label">0</text><line x1="225" y1="180" x2="262.5" y2="250" class="tree-line"></line><text x="233.75" y="215" class="edge-label">1</text><line x1="300" y1="40" x2="450" y2="110" class="tree-line"></line><text x="365" y="75" class="edge-label">1</text><circle cx="300" cy="40" r="20" class="node-circle"></circle><text x="300" y="44" text-anchor="middle" class="node-text">100</text><circle cx="150" cy="110" r="20" class="node-circle"></circle><text x="150" y="114" text-anchor="middle" class="node-text">20</text><circle cx="75" cy="180" r="20" class="node-circle node-leaf"></circle><text x="75" y="184" text-anchor="middle" class="node-text">A:10</text><circle cx="225" cy="180" r="20" class="node-circle"></circle><text x="225" y="184" text-anchor="middle" class="node-text">10</text><circle cx="187.5" cy="250" r="20" class="node-circle node-leaf"></circle><text x="187.5" y="254" text-anchor="middle" class="node-text">D:2</text><circle cx="262.5" cy="250" r="20" class="node-circle node-leaf"></circle><text x="262.5" y="254" text-anchor="middle" class="node-text">B:8</text><circle cx="450" cy="110" r="20" class="node-circle node-leaf"></circle><text x="450" y="114" text-anchor="middle" class="node-text">C:80</text></svg>
                        </div>
                        <!-- Empty State Overlay -->
                        <div id="tree-empty" class="hidden absolute inset-0 bg-white/80 flex items-center justify-center text-slate-400 z-10">
                            数据不足，无法生成树
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="bg-blue-600 p-6 rounded-2xl text-white">
                            <p class="text-xs uppercase opacity-75 mb-1 font-bold">固定长度总位</p>
                            <h4 id="stat-fixed" class="text-3xl font-black">200 bits</h4>
                        </div>
                        <div class="bg-green-600 p-6 rounded-2xl text-white">
                            <p class="text-xs uppercase opacity-75 mb-1 font-bold">哈夫曼带权总长</p>
                            <h4 id="stat-huffman" class="text-3xl font-black">130 bits</h4>
                        </div>
                        <div class="bg-slate-800 p-6 rounded-2xl text-white">
                            <p class="text-xs uppercase opacity-75 mb-1 font-bold">空间节省率</p>
                            <h4 id="stat-savings" class="text-3xl font-black">35.0%</h4>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 gap-12 py-12">
            <div class="order-2 md:order-1">
                <h3 class="text-2xl font-bold mb-4">案例深度分析：小渣 vs 老渣</h3>
                <p class="text-slate-600 mb-6">
                    在您提供的图片案例中，100 道题目的答案分布如下：
                </p>
                <div class="space-y-4">
                    <div class="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-200 shadow-sm">
                        <div class="flex items-center">
                            <div class="w-8 h-8 rounded bg-blue-100 flex items-center justify-center font-bold text-blue-700 mr-3">C</div>
                            <span>80次 (频率最高)</span>
                        </div>
                        <span class="font-mono bg-blue-50 text-blue-700 px-2 rounded">Huffman: 0</span>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-200 shadow-sm">
                        <div class="flex items-center">
                            <div class="w-8 h-8 rounded bg-slate-100 flex items-center justify-center font-bold text-slate-700 mr-3">A</div>
                            <span>10次</span>
                        </div>
                        <span class="font-mono bg-slate-50 text-slate-700 px-2 rounded">Huffman: 10</span>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-200 shadow-sm">
                        <div class="flex items-center">
                            <div class="w-8 h-8 rounded bg-slate-100 flex items-center justify-center font-bold text-slate-700 mr-3">B</div>
                            <span>8次</span>
                        </div>
                        <span class="font-mono bg-slate-50 text-slate-700 px-2 rounded">Huffman: 111</span>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-200 shadow-sm">
                        <div class="flex items-center">
                            <div class="w-8 h-8 rounded bg-slate-100 flex items-center justify-center font-bold text-slate-700 mr-3">D</div>
                            <span>2次</span>
                        </div>
                        <span class="font-mono bg-slate-50 text-slate-700 px-2 rounded">Huffman: 110</span>
                    </div>
                </div>
                <p class="mt-6 text-sm text-slate-500 italic">
                    计算公式：$WPL = \\sum w_i l_i$。在此例中，$80 \\times 1 + 10 \\times 2 + 8 \\times 3 + 2 \\times 3 = 130$ bits，远小于固定编码的 200 bits。
                </p>
            </div>
            <div class="order-1 md:order-2 flex flex-col justify-center">
                <div class="bg-blue-50 p-8 rounded-3xl border border-blue-100">
                    <h4 class="text-xl font-bold text-blue-900 mb-4">为什么叫“最优”？</h4>
                    <p class="text-blue-800 leading-relaxed">
                        哈夫曼算法在数学上证明了：在所有可能的二叉树中，由哈夫曼算法构造出的树具有最小的带权路径长度。这意味着在进行熵编码时，它是理论上最接近信号源熵（信息量极致压缩）的变长编码方式。
                    </p>
                    <img style="" src="blob:https://0kypcw6wli8hcfd6i8ss35ywo1azjj3qoq72nldkitudpiam78-h845251650.scf.usercontent.goog/70290dc0-420c-4a7a-bfcf-308b538c11aa" go-data-src="/gen?prompt=minimalist+diagram+showing+entropy+and+efficiency+curve+data+compression&amp;aspect=16:9" class="mt-6 rounded-xl border border-blue-200">
                </div>
            </div>
        </section>
        ...
        <section class="pb-24">
            <h2 class="text-3xl font-bold text-center mb-12">哈夫曼树在现实中的应用</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition">
                    <span class="material-symbols-outlined text-blue-500 text-4xl mb-4">folder_zip</span>
                    <h4 class="font-bold mb-2">文件压缩</h4>
                    <p class="text-sm text-slate-500">ZIP 和 GZIP 等压缩格式使用哈夫曼编码作为最终的降容手段。</p>
                </div>
                <div class="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition">
                    <span class="material-symbols-outlined text-green-500 text-4xl mb-4">image</span>
                    <h4 class="font-bold mb-2">图像压缩</h4>
                    <p class="text-sm text-slate-500">JPEG 格式在离散余弦变换后使用哈夫曼编码来压缩亮度和色度分量。</p>
                </div>
                <div class="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition">
                    <span class="material-symbols-outlined text-purple-500 text-4xl mb-4">videocam</span>
                    <h4 class="font-bold mb-2">视频传输</h4>
                    <p class="text-sm text-slate-500">MPEG 系列标准中也有哈夫曼树的身影，用于极低延迟的数据流精简。</p>
                </div>
                <div class="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition">
                    <span class="material-symbols-outlined text-red-500 text-4xl mb-4">terminal</span>
                    <h4 class="font-bold mb-2">网络协议</h4>
                    <p class="text-sm text-slate-500">HTTP/2 使用一种静态哈夫曼表（HPACK）来压缩报头，加速网页加载。</p>
                </div>
            </div>
        </section>
    ...
    <script>
        ...
        function initInputs() {
            ...
            data.forEach((item, index) => {
                ...
                div.innerHTML = \`
                    <input type="text" value="\${item.char}" maxlength="1" oninput="updateData(\${index}, 'char', this.value)" 
                           class="w-12 px-2 py-2 border rounded-lg text-center font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                    <input type="number" value="\${item.weight}" oninput="updateData(\${index}, 'weight', this.value)" 
                           class="flex-grow px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    <button onclick="removeRow(\${index})" class="text-slate-400 hover:text-red-500 transition">
                        <span class="material-symbols-outlined">delete</span>
                    </button>
                \`;
        ...
        function addRow() {
            const charCode = 65 + data.length; // A, B, C...
            data.push({ char: String.fromCharCode(charCode), weight: 5 });
            initInputs();
        }

        function removeRow(index) {
            if (data.length <= 2) return alert('哈夫曼树至少需要2个节点');
            data.splice(index, 1);
            initInputs();
        }
        ...
        function updateData(index, key, value) {
            if (key === 'weight') {
                data[index].weight = parseInt(value) || 0;
            } else {
                data[index].char = value.toUpperCase() || '?';
            }
            refresh();
        }

        function buildHuffmanTree(items) {
            let forest = items.map(d => new Node(d.char, d.weight));
        ...
        function updateTable(codes) {
            ...
            const fixedBitLen = Math.ceil(Math.log2(data.length));
            
            data.forEach(item => {
                const tr = document.createElement('tr');
                tr.innerHTML = \`
                    <td class="px-3 py-2 font-bold">\${item.char} <span class="text-xs font-normal text-slate-400">(\${item.weight})</span></td>
                    <td class="px-3 py-2 font-mono text-slate-400">\${item.char.charCodeAt(0).toString(2).padStart(fixedBitLen, '0').slice(-fixedBitLen)}</td>
                    <td class="px-3 py-2 font-mono text-blue-600 font-bold">\${codes[item.char] || ''}</td>
                \`;
        ...
        function updateStats(codes) {
            ...
            const fixedBitLen = Math.ceil(Math.log2(data.length));
            const totalFixedBits = totalWeight * fixedBitLen;
            
            let totalHuffmanBits = 0;
            data.forEach(item => {
                totalHuffmanBits += item.weight * (codes[item.char]?.length || 0);
            });

            const savings = totalFixedBits > 0 ? ((totalFixedBits - totalHuffmanBits) / totalFixedBits * 100).toFixed(1) : 0;

            document.getElementById('stat-fixed').innerText = \`\${totalFixedBits} bits\`;
            document.getElementById('stat-huffman').innerText = \`\${totalHuffmanBits} bits\`;
            document.getElementById('stat-savings').innerText = \`\${savings}%\`;
        }

        // SVG Tree Rendering
        function drawTree(root) {
            const svg = document.getElementById('huffman-svg');
            ...
            function traverse(node, depth, xStart, xEnd, xPos) {
                if (!node) return;
                ...
                if (node.left) {
                    const nextX = xStart + (xPos - xStart) / 2;
                    edges.push({ x1: xPos, y1: y, x2: nextX, y2: y + levelHeight, label: '0' });
                    traverse(node.left, depth + 1, xStart, xPos, nextX);
                }
                if (node.right) {
                    const nextX = xPos + (xEnd - xPos) / 2;
                    edges.push({ x1: xPos, y1: y, x2: nextX, y2: y + levelHeight, label: '1' });
                    traverse(node.right, depth + 1, xPos, xEnd, nextX);
                }
            }

            traverse(root, 0, 0, width, width / 2);
            ...
            edges.forEach(e => {
                ...
                label.setAttribute("x", (e.x1 + e.x2) / 2 - 10);
                label.setAttribute("y", (e.y1 + e.y2) / 2);
            ...
            nodes.forEach(n => {
                ...
                circle.setAttribute("class", n.isLeaf ? "node-circle node-leaf" : "node-circle");
                ...
                text.textContent = n.isLeaf ? \`\${n.char}:\${n.weight}\` : n.weight;
...
// Resolve local asset paths for the iframe
let finalHtml = htmlCode;
// Note: We use the local paths that were already substituted into finalHtml
// But we still need the mapping to find the vault files.
const localPaths = ["資料結構/assets/huffman/AF2bZyjKMgp8hYtrz8uwlAs5nm2p_PlQ7vIqFy1VAru7NqEk7G.jpg", "資料結構/assets/huffman/img_57f320b3bb.jpg", "資料結構/assets/huffman/img_b619eb1bb8.jpg", "資料結構/assets/huffman/unnamed(1).jpg", "資料結構/assets/huffman/unnamed(2).jpg", "資料結構/assets/huffman/unnamed(3).jpg", "資料結構/assets/huffman/unnamed(4).jpg", "資料結構/assets/huffman/unnamed(5).jpg", "資料結構/assets/huffman/unnamed.jpg"];

for (const localPath of localPaths) {
    const file = app.vault.getAbstractFileByPath(localPath);
    if (file) {
        const resourcePath = app.vault.adapter.getResourcePath(file.path);
        finalHtml = finalHtml.split(localPath).join(resourcePath);
    }
}
...
```dataviewjs
// Enhanced DSU Simulation for Obsidian
const htmlCode = String.raw`
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
  ...
  <style>
    body { margin: 0; background: transparent; font-family: sans-serif; overflow: hidden; }
    .node-transition { transition: all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
    .scrollbar-thin::-webkit-scrollbar { width: 4px; }
    .scrollbar-thin::-webkit-scrollbar-thumb { background: #334155; border-radius: 10px; }
    .code-line { transition: all 0.3s ease; border-left: 3px solid transparent; }
    .code-highlight { background: rgba(59, 130, 246, 0.15); border-left-color: #3b82f6; color: #60a5fa; font-weight: bold; }
  </style>
</head>
<body class="bg-slate-50">
  <div class="flex flex-col h-screen p-4 gap-4">
    <div class="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
      <div class="flex gap-2">
        <button id="union-btn" onclick="runUnion()" class="flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-xl hover:bg-blue-700 disabled:opacity-50 transition-all shadow-md active:scale-95">
          <span class="material-symbols-outlined text-[20px]">join_inner</span> 合併 (Union)
        </button>
        <button id="find-btn" onclick="runFind()" class="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2 rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-all shadow-md active:scale-95">
          <span class="material-symbols-outlined text-[20px]">search</span> 查詢 (Find)
        </button>
        <button onclick="resetDSU()" class="flex items-center gap-2 bg-slate-100 text-slate-600 px-5 py-2 rounded-xl hover:bg-slate-200 transition-all active:scale-95">
          <span class="material-symbols-outlined text-[20px]">restart_alt</span> 重置
        </button>
      </div>
      <div id="status-panel" class="text-xs font-bold text-slate-400 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex items-center gap-3">
        <span class="flex items-center gap-1"><span class="w-2 h-2 bg-blue-500 rounded-full"></span>選取: <span id="selected-nodes-text" class="text-slate-700">None</span></span>
      </div>
    </div>

    <div class="flex-1 flex gap-4 min-h-0">
      <!-- Visualizer Area -->
      <div class="flex-1 bg-white rounded-[2rem] shadow-inner border border-slate-200 relative overflow-hidden flex items-center justify-center">
        <svg id="visualizer-svg" width="100%" height="100%" class="w-full h-full"></svg>
      </div>
      
      <!-- Right Panel: Code + Logs -->
      <div class="w-[320px] flex flex-col gap-4">
        <!-- Code Module -->
        <div class="flex-1 bg-slate-900 rounded-3xl p-5 shadow-2xl border border-slate-800 flex flex-col overflow-hidden">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
              <span class="material-symbols-outlined text-[14px]">code</span> Algorithm Code
            </h3>
            <span class="text-[9px] text-slate-600 font-mono">dsu_impl.cpp</span>
          </div>
          <div id="code-content" class="flex-1 font-mono text-[11px] leading-[1.8] text-slate-400 overflow-y-auto pr-2">
            <div id="line-07" class="code-line px-3"><span class="text-slate-600 mr-3">07</span><span class="text-indigo-400">int</span> Find(<span class="text-indigo-400">int</span> x) {</div>
            <div id="line-08" class="code-line px-3"><span class="text-slate-600 mr-3">08</span>  <span class="text-pink-400">if</span> (parent[x] != x)</div>
            <div id="line-09" class="code-line px-3"><span class="text-slate-600 mr-3">09</span>    parent[x] = Find(parent[x]);</div>
            <div id="line-10" class="code-line px-3"><span class="text-slate-600 mr-3">10</span>  <span class="text-pink-400">return</span> parent[x];</div>
            <div id="line-11" class="code-line px-3 mb-3"><span class="text-slate-600 mr-3">11</span>}</div>
            
            <div id="line-12" class="code-line px-3"><span class="text-slate-600 mr-3">12</span><span class="text-indigo-400">void</span> Union(<span class="text-indigo-400">int</span> x, <span class="text-indigo-400">int</span> y) {</div>
            <div id="line-13" class="code-line px-3"><span class="text-slate-600 mr-3">13</span>  <span class="text-indigo-400">int</span> fx = Find(x), fy = Find(y);</div>
            <div id="line-14" class="code-line px-3"><span class="text-slate-600 mr-3">14</span>  <span class="text-pink-400">if</span> (fx == fy) <span class="text-pink-400">return</span>;</div>
            <div id="line-15" class="code-line px-3"><span class="text-slate-600 mr-3">15</span>  <span class="text-pink-400">if</span> (rank[fx] < rank[fy]) parent[fx] = fy;</div>
            <div id="line-16" class="code-line px-3"><span class="text-slate-600 mr-3">16</span>  <span class="text-pink-400">else</span> {</div>
            <div id="line-17" class="code-line px-3"><span class="text-slate-600 mr-3">17</span>    parent[fy] = fx;</div>
            <div id="line-18" class="code-line px-3"><span class="text-slate-600 mr-3">18</span>    <span class="text-pink-400">if</span> (rank[fx] == rank[fy]) rank[fx]++;</div>
            <div id="line-19" class="code-line px-3"><span class="text-slate-600 mr-3">19</span>  }</div>
            <div id="line-20" class="code-line px-3"><span class="text-slate-600 mr-3">20</span>}</div>
          </div>
        </div>

        <!-- Log Content -->
        <div class="h-32 bg-slate-900 rounded-3xl p-5 shadow-inner border border-slate-800 flex flex-col">
          <div id="log-container" class="flex-1 overflow-y-auto pr-2 space-y-1.5 text-[10px] font-mono text-slate-500">
            <div class="text-blue-500 opacity-60 italic">> System Ready...</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function initPositions() {
      ...
      const centerX = container.width ? container.width / 2 : 300;
      const centerY = container.height ? container.height / 2 : 200;
      const radius = Math.min(centerX, centerY) * 0.75;
      nodesData.length = 0;
      for (let i = 0; i < N; i++) {
        const angle = (i / N) * 2 * Math.PI - Math.PI/2;
        nodesData.push({ id: i, x: centerX + radius * Math.cos(angle), y: centerY + radius * Math.sin(angle) });
      }
    }

    function addLog(msg, color = 'text-slate-300') {
      const div = document.createElement('div');
      div.className = color + ' border-l border-slate-700 pl-2';
      div.textContent = '> ' + msg;
    ...
    function setHighlight(lines) {
      document.querySelectorAll('.code-line').forEach(l => l.classList.remove('code-highlight'));
    ...
    function render() {
      ...
      dsu.parent.forEach((p, i) => {
        if (p !== i) {
          const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
          line.setAttribute('x1', nodesData[i].x); line.setAttribute('y1', nodesData[i].y);
          line.setAttribute('x2', nodesData[p].x); line.setAttribute('y2', nodesData[p].y);
          line.setAttribute('stroke', '#cbd5e1'); line.setAttribute('stroke-width', '2.5');
          line.setAttribute('stroke-linecap', 'round');
          line.classList.add('node-transition');
          svg.appendChild(line);
        }
      });
      ...
      nodesData.forEach((node, i) => {
        ...
        g.className.baseVal = 'cursor-pointer';
        g.onclick = () => !isAnimating && toggleNode(i);

        const circle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        circle.setAttribute('cx', node.x); circle.setAttribute('cy', node.y);
        circle.setAttribute('r', isSelected ? '22' : '20');
        circle.setAttribute('fill', isSelected ? '#3b82f6' : (isRoot ? '#1e293b' : '#94a3b8'));
        circle.setAttribute('stroke', isSelected ? '#bfdbfe' : '#ffffff');
        circle.setAttribute('stroke-width', isSelected ? '4' : '2');
        circle.classList.add('node-transition');
        
        const text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        text.setAttribute('x', node.x); text.setAttribute('y', node.y + 6);
        text.setAttribute('text-anchor', 'middle'); text.setAttribute('fill', 'white');
        text.setAttribute('class', 'text-[12px] font-black select-none');
        text.textContent = i;

        if (isRoot) {
            const rTxt = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            rTxt.setAttribute('x', node.x); rTxt.setAttribute('y', node.y - 28);
            rTxt.setAttribute('text-anchor', 'middle'); rTxt.setAttribute('fill', '#94a3b8');
            rTxt.setAttribute('class', 'text-[9px] font-bold tracking-tighter');
            rTxt.textContent = 'RANK:' + dsu.rank[i];
            g.appendChild(rTxt);
        }
      ...
      document.getElementById('union-btn').disabled = selectedNodes.length !== 2 || isAnimating;
      document.getElementById('find-btn').disabled = selectedNodes.length !== 1 || isAnimating;
    }

    function toggleNode(idx) {
      const pos = selectedNodes.indexOf(idx);
      if (pos > -1) selectedNodes.splice(pos, 1);
      else {
        if (selectedNodes.length >= 2) selectedNodes.shift();
        selectedNodes.push(idx);
      }
      document.getElementById('selected-nodes-text').textContent = selectedNodes.length > 0 ? selectedNodes.join(', ') : 'None';
      render();
    }
    ...
    async function runFind() {
      const u = selectedNodes[0];
      isAnimating = true;
      selectedNodes = [];
      
      addLog('Calling Find(' + u + ')', 'text-indigo-400 font-bold');
      setHighlight(['07', '08']);
      await sleep(800);

      const root = dsu.find(u);
      setHighlight(['09']);
      addLog('Path Compression active...', 'text-amber-500');
      animateNodes();
      await sleep(1000);

      setHighlight(['10']);
      addLog('Found Root: ' + root, 'text-green-500 font-bold');
      await sleep(600);
    ...
    async function runUnion() {
      ...
      addLog('Calling Union(' + u + ', ' + v + ')', 'text-blue-400 font-bold');
      setHighlight(['12', '13']);
      await sleep(800);

      const fx = dsu.find(u), fy = dsu.find(v);
      setHighlight(['14']);
      await sleep(600);

      if (fx === fy) {
        addLog('Already connected!', 'text-slate-500');
        setHighlight(['14']);
      } else {
        addLog('Merging sets...', 'text-blue-400');
        setHighlight(['15', '16', '17', '18', '19']);
        dsu.union(u, v);
        animateNodes();
      }
    ...
    function resetDSU() {
      dsu = new DSU(N); selectedNodes = []; setHighlight([]);
      addLog('DSU Reset.', 'text-slate-400');
      render();
    }
    ...
    function animateNodes() {
      ...
      function step() {
        ...
        dsu.parent.forEach((p, i) => {
          if (p === i) return;
          const n = nodesData[i], par = nodesData[p];
          const dx = par.x - n.x, dy = par.y - n.y;
          if (Math.sqrt(dx*dx + dy*dy) > 55) {
            n.x += dx * 0.1; n.y += dy * 0.1;
            changed = true;
          }
        });
        render();
        if (changed && frames++ < 60) requestAnimationFrame(step);
      }
      requestAnimationFrame(step);
    }

    setTimeout(() => { initPositions(); render(); }, 300);
...
dv.el("iframe", "", {
    attr: {
        srcdoc: htmlCode,
        style: "width: 100%; height: 750px; border: none; border-radius: 32px; background: white; box-shadow: 0 20px 50px -12px rgba(0, 0, 0, 0.15);"
    }
});
```
