...
<body>
    ...
    <script type="text/babel">
        ...
        const App = () => {
            ...
            const [logs, setLogs] = useState(["準備就緒..."]);

            const steps = useMemo(() => {
                const s = []; let curR = []; let curT = [];
                const findNode = (r, id) => {
                ...
                while (curr) {
                    ...
                    if (!curr.left) {
                        curR.push(curr.val);
                        s.push({ c: curr.id, p: null, ts: [...curT], r: [...curR], m: curr.val + " 無左子樹，移至右側" });
                        curr = getR(curr, curT);
                    } else {
                        let p = curr.left;
                        ...
                        if (!getR(p, curT)) {
                            ...
                            curR.push(curr.val);
                            s.push({ c: curr.id, p: p.id, ts: [...curT], r: [...curR], m: "斷開線索，訪問 " + curr.val });
                            curr = getR(curr, curT);
                        }
                    }
                }
            ...
            const handleNext = () => {
                setStepIdx(prev => {
                    ...
                    if (next < steps.length) {
                        const s = steps[next];
                        setCurrentId(s.c); setPreId(s.p); setThreads(s.ts); setRes(s.r);
                        setLogs(p => [s.m, ...p].slice(0, 3));
                        return next;
                    }
                    setIsPlaying(false);
                    return prev;
            ...
            useEffect(() => {
                if (isPlaying) {
                    const t = setInterval(handleNext, 800);
            ...
            return (
                <div className="p-4 bg-white flex flex-col h-full">
                    <div className="flex justify-between items-center mb-4">
                        <div className="text-sm font-bold">Morris Traversal 視覺化</div>
                    ...
                    <div className="flex-1 bg-slate-50 rounded-lg flex justify-center items-center overflow-hidden mb-4">
                        <svg width="100%" height="240" viewBox="0 0 400 300">
                            ...
                            {threads.map((t, i) => {
                                ...
                                if (!f || !to) return null;
                                return <path key={i} d={"M"+f.x+" "+f.y+" Q"+(Math.max(f.x, to.x)+40)+" "+((f.y+to.y)/2)+" "+to.x+" "+to.y} fill="none" stroke="#10b981" strokeWidth="2" strokeDasharray="4" />;
                            })}
                        </svg>
                    </div>
                    <div className="bg-slate-900 text-blue-300 p-2 rounded font-mono text-xs mb-2">
                        {logs.map((log, i) => <div key={i} className={i === 0 ? "text-green-400" : "opacity-50"}>{log}</div>)}
                    </div>
                </div>
            );
        };
...
// 使用 srcdoc 避免 Blob URL 被攔截
dv.el("iframe", "", {
    attr: {
        srcdoc: htmlCode,
        style: "width: 100%; height: 550px; border: none; border-radius: 12px; background: white;"
    }
});
```
